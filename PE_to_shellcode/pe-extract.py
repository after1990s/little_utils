...
def load_section_contents(pe_load_file, all_sections, section, section_name):
    if section.Name.find(section_name) == 0:
        data = pe_load_file.get_data(section.VirtualAddress, section.SizeOfRawData)
        all_sections[section_name] = {'vastart': section.VirtualAddress,
                                      'mem_size': section.Misc_VirtualSize,
                                      'file_size':section.SizeOfRawData,
                                      'data': data}
        print "%s_start: 0x%.8x, %s_size: 0x%.4x" % (
        section_name, section.VirtualAddress, section_name, section.Misc_VirtualSize)
...
def load_pe_sections(pe_load_file):
    ...
    sections_list = ['\x00' for _ in range(0, pe_load_file.OPTIONAL_HEADER.SizeOfImage)]#记录所有除section的数据

    for i in sections:
        sections_list_begin = sections[i]['vastart']
        sections_list_end = sections[i]['vastart']+sections[i]['file_size']
        sections_list[sections_list_begin:sections_list_end] = sections[i]['data']
    #sections_list = sections_list[pe_load_file.OPTIONAL_HEADER.AddressOfEntryPoint:]
    sections_list = sections_list[sections['.text']['vastart']:]
    for i in sections:
        sections[i]['offset'] = sections[i]['vastart'] - sections['.text']['vastart']
        if (sections[i]['offset'] < 0):
            print "pe entry addr:" + str(pe_load_file.OPTIONAL_HEADER.AddressOfEntryPoint)
            print str(sections[i]['vastart'])
        assert(sections[i]['offset'] >= 0)
    # next_begin_offset = 0
    #
...
def calc_reloc_entry_offset_in_sections_list(reloc_item_rva, sections):
    ...
    for i in sections:
        ...
        if section_begin <= rva and rva <= section_end:
            offset = rva - section_begin + sections[i]['offset']
            if(offset < 0):
        #         print "rva" + str(rva)
        #         print str(section_begin)
        #         print str(section_end)
        #         print str(sections[i]['offset'])
                assert(false)
            return offset
        else:
            continue
        ...
        for entry in relocs.entries:
            ...
            for i in sections:
                ...
                if try_add_offset_to_reloc_item_addr(reloc_addr_offset,
                    ...
                    #print "offset:" + str(reloc_item_offset_from_shellcode)
                    #print ("list: ")  + str(sections_list[reloc_item_offset_from_shellcode:reloc_item_offset_from_shellcode+4])
                    ...
                    if reloc_item_va != string_to_dword(
                        ...
                        print "excepted: " + str(reloc_item_va)
...
def add_loader_to_shellcode_v2(pe_load_file, sections,reloc_items, sections_list, reloc_addr_offset):
    jump_space = pe_load_file.OPTIONAL_HEADER.AddressOfEntryPoint - sections['.text']['vastart']
    
    print "jmp space: 0x%.8x" % (jump_space)
    print "pe_entry dword: 0x%.8x" % (pe_load_file.get_dword_at_rva(pe_load_file.OPTIONAL_HEADER.AddressOfEntryPoint))
    jump_space = dword_to_bytes(jump_space)
    if reloc_items == 0:
    ...
    code = [
        ...
        0x90, 
        0xE9, jump_space[0], jump_space[1], jump_space[2], jump_space[3]
                                                    # shellcode_start:
    ] # loader must align of memory.
...
def main(pe_name, output_name):
    ...
    assert(len(sections_list) + sections['.text']['vastart'] == pe_load_file.OPTIONAL_HEADER.SizeOfImage)
    ...
    r_shellcode = add_loader_to_shellcode_v2(pe_load_file, sections,reloc_item,sections_list, reloc_addr_offset)
...
if __name__=='__main__':
    ...
    #main("D:\\FG\\shellcode\\replace_peimage\\replace_peimage\\Release\\replace_peimage.ee","D:\\FG\\shellcode\\pe_to_shellcode\\messagebox.bin")
    #main("D:\\FG\\fg_server\\Release\\fg_server.exe","D:\\FG\\fg_server\\Release\\Mc.cp")
    main("F:\\ctm\\SafeSvc\\Release\\Choiceguard.exe", "F:\\ctm\\SafeSvc\\Release\\msi.hpp")
